...
\subsection{Theory}
    ...
    - Give Magnus expansion, and commutations (check SS and OB thesis theory sections)\todo{I think this should be a citation and an appendix}
    ...
    \todo{Move following details to appendix.}
    ...
    expectation of observable $\hat O$ on the oscillator, $\mathrm{Tr}(\hat O\rho_\mathrm{osc})$, 
    via exclusive measurements of the ancilla qubit. Full
...
\subsection{Experimental Demonstration}
    ...
    apparatus we were limited to global addressing fields, limiting the
    application of these hybrid interactions to one- and two-qubits. Here we
...
\subsubsection{Two Spin Dependent Forces}
    ...
    different modes. \todo{Add colour to 2-sdf plot, could also make two
    examples for BS and sqz?} 
    Similar to the generation of the bichromatic field, we may produce this
    ...
    As discussed in section~\ref{sec:sdf}, the spin phase of the SDFs is set by
    ...
    $t_ramp$ is the ramp duration. This ramp suppresses excess off-resonant
    ...
    $\frac{1}{\Delta}$. Here, we set the base detuning,
    $\Delta=50~\unit{\kilo\hertz}$, and use a ramp duration
    $t_\mathrm{ramp}=40~\unit{\micro\second}$.\\
...
\subsubsection{Squeezing}
    ...
    - tickling calibration

    We demonstrate the effective squeezing interaction on a single ion and radial
    motional mode initiated in the vacuum state. We are limited to using the
    ...
    $\omega_{upper}=XX~\unit{\mega\hertz}$, and motional coherence given in
    section~\ref{sec:motional_coherence}.
    Squeezing is implemented by the
    quadchromatic field, with total beam power of $40~\unit{\micro\watt}$, and
    $m=-1$. The state is characterised by the tomography method given in
    section~\ref{sec:tomography}. The tomography SDF strength,
    $\Omega_\mathrm{split}$ is independently calibrated through on-resonance
    splitting of a vacuum state to find, $\Omega_\mathrm{split}=2\pi
    3.6(2)~\unit{\kilo\hertz}$. This strength is accounted for in the
    displacement duration, $t_\mathrm{split}$, to provide the correct scaling
    factor for the characteristic function axes, $t_\mathrm{split} =
    \frac{|\beta|}{2\Omega_\mathrm{split}}$. 
    ...
    The plot corresponds to scanning over a $35\times 35$ grid of displacement
    values, $\beta$, each point corresponding to the average over $300$ samples.
    Fitting the data to equation~\ref{eqn:sqz_char} we may extract both the
    squeezing axis, $\phi$, and the strength, $r$. An example fit is shown in
    panel (b), with extracted $r=0.85(2)$, for a quadchromatic FWHM pulse
    duration of $t_\mathrm{sqz}=190~\unit{\micro\second}$. Panel (c) displays
    the squeezing strength as a function $t_\mathrm{sqz}$, showing the expected
    linear relationship between squeezing duration and $r$.\\
...
\subsection{Outlook}
    ...
    \todo{Standing wave extension}\\
    \todo{Limitations: need to SBC all modes we want to use; readout of mode is expensive due to requiring map to spin;}
