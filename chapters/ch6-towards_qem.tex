...
\subsection{Theory}
    ...
    The two simultaneous SDFs, with Hamiltonians and parameters labelled by subscripts $\alpha$, and $\beta$, are applied off-resonantly relative to motional modes with creation operators $\hat a^\dagger$ and $\hat b^\dagger$, respectively. The resulting Hamiltonian is given generally by,
    \begin{align}
    \label{eqn:two-sdf}
    ...
    With the above methods we may generate interesting bosonic states, now we
    provide a method for characterising them.\\
    In contrast to a two-level system, the state of an oscillator may not be
    simply directly readout. In part this is due to the much larger state-space
    a the qumode, but also, we utilise an indirect readout via an ancillary
    qubit, followed by typical state-dependent fluorescence
    (section~\ref{sec:fluo}).\\
    The information is mapped to the ancilla by a Hadamard test~\cite{}, shown in
    figure~\ref{fig:tomography} (a). Here the expectation of observable $\hat O$ on the oscillator, $\mathrm{Tr}(\hat O\rho_\mathrm{osc})$, 
    is recovered through exclusive measurements of the ancilla qubit after qumode-dependent phase kicks are applied. 
    Full state tomography may now be formulated by repeat measurements of the conditional displacement operation, 
    $\hat O = e^{-i\theta}D(\beta)$, to recover the characteristic function, defined as $\chi =
    \mathrm{Tr}(D(\beta)\rho_\mathrm{osc})$, where $\beta$ is a complex parameter giving both displacement magnitude and angle.
    ...
    \end{align}
    where $\theta$ may be set to $0$ or $\pi/2$ to measure the real of imaginary part of the characteristic function respectively.
    Each measurement provides only 1-bit of information on the (continuous) oscillator state, thus 
    many samples with a range of $\beta$ values must be taken to approximate the state.\\
    Once the characteristic function has been measure, it can be converted to other familiar representations of the oscillator, such as the Fock state~\cite{}, or various types in phase-space, including the Wigner representation~\cite{}. In this work, we shall keep the the conveniently measured characteristic function.\\
    ...
    \end{equation}
    while the squeezed state, with squeezing parameter $\xi = re^{i\phi}$, breaks this symmetry due to a contraction on one axis, and a corresponding expansion on the other,
    \begin{equation}
        \label{eqn:sqz_char}
    ...
    Experimentally the $\hat\sigma_x$ conditioned spin-dependent force are simply accessed by the bichromatic field method discussed in section~\ref{sec:SDF}. The Hadamard test circuit is decomposed to native gates, shown in
    figure~\ref{fig:tomography} (b), and first described in
    reference~\cite{fluhmann_direct_2020}. The proceeding $I$ or
    $R_y(\pi/2)$ provide the quadrature to measure; $\mathrm{Re}(\chi)$ or $\mathrm{Im}(\chi)$
    respectively.
    ...
    section~\ref{sec:k-copy_char}, full tomographic reconstruction is
    ...
    qubits and qumodes, either small regions of the characteristic function must
    be selected~\cite{} or simplified metrics of performance~\cite{}\todo{ting
    rei bosonic RB}, rather than tomography must be utilised.
...
\subsection{Outlook}
    ...
    information between these chains either through ion transport maintaining mode and spin coherence~\cite{walther_controlling_2012},
    optical networks~\cite{}\todo{HOA2?}, or through inducing collective interactions
