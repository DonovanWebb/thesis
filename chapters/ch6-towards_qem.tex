...
Hybrid spin-oscillator computation has been studied in the context of superconducting cavity-QED~\cite{}, neutral atom in optical tweezers~\cite{}, and in the trapped-ion plaform~\cite{}. 
...
The qumodes may be directly driven through trap potential modulation~\cite{}, or externally applied RF fields~\cite{}; the qubit may be driven either by optical fields (in the case of optical qubit encoding), or through microwaves (in the case of ground state encoding); and 
the qubit and qumode may be coupled through qubit-state dependent potential shifts~\cite{}, or through optical fields tuned near to motional-mode frequency sidebands of the qubit-carrier transition~\cite{}.
...
\subsection{Theory}
    ...
    For any computing paradigm, we must establish a set of interactions required for universality. In DV-only systems, quantum universality may be realised by a finite set of Clifford operations plus one non-Clifford gate~\cite{nielsen_quantum_2010}. CV-only universality can analogously be realised by a finite set of Gaussian operations plus one non-Gaussian gate~\cite{}.
    The term Gaussian is best understood by the position-momentum representation of the qumode, where the bare oscillator Hamiltonian is given by,
    \begin{equation}
    ...
    Specifically, we consider a cubic CV instruction set~\cite{lloyd_quantum_1999, eriksson_universal_2024, hillmann_universal_2020}, taking the Gaussian operations and supplementing with a tri-squeezing gate, with Hamiltonian,
    \begin{equation}
    ...
    The squeezing, parity, and beam-splitter interactions may be generated by the same method, and have been experimentally demonstrated~\cite{bazavan_squeezing_2024,saner_real-time_2025,bazavan_easy_nodate,saner_quantum_nodate}.
    The displacement operation can be realised either spin-independently by RF fields near resonance to the motional-mode frequencies (section~\ref{sec:tickle}), or spin-conditioned using various SDF implementations, such as through a bichromatic field (section~\ref{sec:sdf}), through the static Stark-shift gradients due to an optical tweezer~\cite{leindecker_state_2026}, or by AC Stark-shifts due to counterpropagating Raman beams~\cite{}.\\
    \todo{Subsection needs summary/transfer sentence.}
    ...
    % - Intro representations of a mode. Number and quadrature, mention phase
    % space, wigner, others.\\
    % - For CV gateset need Gaussian and at least one non-Gaussian operation (c.f.
    % Clifford and non-Clifford gates), Wigner negativity. See table III.1 in
    % Liu.\\
    % - Define gaussian as at most quadratic in $x$ and $p$.\\
    % - Freedom to choose the native operations, we present a particularly
    % accessible set given light-matter interactions available on trapped ions.
    % This is a cubic instruction set (Can expand details from Liu III.E.2). \\
    - Quick overview of the interactions and where we have demonstrated them.
    Could give flow fields for single-mode operators (could be a nice figure,
    and I like that view).\\
    - End section by quick discussion on hybrid system, as our generated
    interactions have spin dependence.\\
    \begin{table}
        \begin{center}
    ...
    The two simultaneous SDFs, here labelled $\mathrm{SDF}_\alpha$, and $\mathrm{SDF}_\beta$, are applied off-resonantly relative to motional modes with creation operators $\hat a^\dagger$ and $\hat b^\dagger$, respectively. The resulting Hamiltonian is given generally by,
    \begin{align}
    \label{eqn:two-sdf}
      \hat H_\mathrm{2SDF}(t)  &= \hat H_\alpha(t) + \hat H_\beta(t)\\
      &=
      \frac{\hbar \Omega_\alpha}{2}\hat\sigma_\alpha \left(\hat a e^{i(\Delta_\alpha t + \phi_\alpha)} + \hat a^\dagger e^{-i(\Delta _\alpha t + \phi_\alpha )}\right)\\
      &~+ 
      \frac{\hbar \Omega_{\beta}}{2}\hat\sigma_{\beta} \left(\hat b e^{i(\Delta_\beta t + \phi_\beta)} + \hat b^\dagger e^{-i(\Delta_\beta t + \phi_\beta)}\right),
    \end{align}
    where the SDF strength is $\Omega_i$, the detuning from each mode is $\Delta_i$, the motional phase is $\phi_i$, and we ensure the spin-conditioning anticommute $\{\hat\sigma_\alpha, \hat\sigma_\beta\} = 0$.
    The unitary dynamics of $\hat H_\mathrm{2SDF}$ may be evaluated by the Magnus expansion~\cite{magnus_exponential_1954}, with details given in references~\cite{sutherland_universal_2021,bazavan_easy_nodate,saner_quantum_nodate}, and in appendix~\ref{app:magnus}. 
    The Magnus expansion expresses the generator of the unitary evolution as an infinite series of terms, with successive terms proportional to increasingly higher-order nested commutators of the Hamiltonian evaluated at different times. Due to the non-commuting spin conditioning, this series does not truncate, leading to highly non-linear bosonic terms effectively driven via the application of the linear SDFs.\\
    To utilise specific non-linear terms within this infinite series, matching the desired interactions given in table~\ref{tab:gateset}, we constrain the relative detunings of the two SDFs to $\Delta_\beta = m \Delta_\alpha$, where $m$ is some integer given in the table. This immediately brings components of the Magnus terms matching the desired non-linear $\hat H_\mathrm{osc}$ into resonance. Furthermore, non-resonant and higher-order terms of the expansion may be arbitrarily suppressed through ensuring $\Delta \gg \Omega_\alpha\Omega_\beta$. In this regime, the unitary dynamics of $\hat H_\mathrm{2SDF}$ are well approximated by the evolution of time-independent effective Hamiltonians, which for Gaussian operations ($|m| = 1$) are of the form,
    \begin{equation}
        \hat H_\mathrm{eff,G} = \frac{\Omega_\alpha\Omega_\beta}{\Delta} 2i\epsilon_{\alpha\beta\gamma}\hat\sigma_\gamma a^{(\dagger)}b^{(\dagger)} + h.c.,
    \end{equation}
    where $\epsilon_{\alpha\beta\gamma}$ is the Levi-Civita symbols. If $a=b$, the SDFs are detuned from the same motional mode, leading to squeezing, or parity effective interactions, while acting on two-modes leads to the beam splitter interaction.
    For the tri-squeezing interaction ($m=-2$) the effective Hamiltonian is,
    \begin{equation}
        \hat H_\mathrm{eff,T} = \frac{\Omega^2_\alpha\Omega_\beta}{2\Delta^2} 4\hat\sigma_\beta a^3 + h.c.,
    \end{equation}
    where we have set $a=b$, as we act on a single mode.\\
    Frequency and spin-conditioning control of two SDFs therefore allow for the generation of the entire proposed hybrid CV-DV gate-set. After a discussion on how we can characterise the oscillator state, we shall present the experimental scheme for generating these interactions, and demonstrate the creation of a squeezed state by this method.
...
\subsection{Outlook}
    ...
    \todo{Limitations: Initialisation need to SBC all modes we want to use; readout of mode is expensive due to requiring map to spin; Spectral crowding of modes}
