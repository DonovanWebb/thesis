...
\subsection{Theory}
    ...
    \begin{equation}
        \hat H_\mathrm{eff,G} = \frac{\hbar\Omega_\alpha\Omega_\beta}{2\Delta} \epsilon_{\alpha\beta\gamma}\hat\sigma_\gamma (iab^{(\dagger)} + h.c.),
    \end{equation}
    \todo{need to add back phases. }
    where $\epsilon_{\alpha\beta\gamma}$ is the Levi-Civita symbols. If $a^\dagger=b^\dagger$, the SDFs are detuned from the same motional mode, leading to squeezing, or parity effective interactions, while acting on two-modes leads to the beam splitter interaction.
    For the tri-squeezing interaction ($m=-2$) the effective Hamiltonian is,
    \begin{equation}
        \hat H_\mathrm{eff,T} = \frac{\hbar\Omega^2_\alpha\Omega_\beta}{4\Delta^2} \hat\sigma_\beta (a^3 + a^{\dagger 3}),
    \end{equation}
    where we have set $a^\dagger=b^\dagger$, as we act on a single mode.\\
...
\subsubsection{Squeezing}
    - Detuning and power selected (only 53 uW!), and predicted strength of interaction?\\
    ...
    $\omega_{upper}=2.87~\unit{\mega\hertz}$, and motional coherence given in
    section~\ref{sec:motional_coherence}.
    The initial near-vacuum motional state is prepared by Doppler and sideband cooling, with a typical thermal fock state distribution with mean $\bar{n} = 0.03(1)$\todo{Check for more recent values}, as described in section~\ref{sec:cooling}. The spin state is initialised into the ground state by optical pumping, described in section~\ref{sec:optical_pump}.
    ...
    section~\ref{sec:tomography}, with a discretization set to collect a $35\times 35$ grid of displacement
    values, $\beta$, and $N = 300$ repeat samples to reduce shot noise. The full dataset for one of the tomographic scans takes around 1 hour to collect, and so we interleave mode calibration experiments (section~\ref{sec:tickle}) to run every 5 minutes.\\
...
\subsection{Outlook}
    ...
    information between these chains either through qubit transport~\cite{},
    optical networks~\cite{}, or through inducing collective interactions
    between separate ion-chains~\ref{valentini_demonstration_2025}. \\
    Another
