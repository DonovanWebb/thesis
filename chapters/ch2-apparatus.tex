...
% Beam paths? AOM DP and SP?which are frequency stabilised to a reference cavity\footnote{\emph{Stable Laser Systems} SLS-6010 4-Bore Cylindrical Cavity} via Pound-Drever-Hall (PDH) locking. 
    ...
    to $<1~\unit{\mega\hertz}$ level, well below the natural line widths of the dipole transitions, and power stable to $<1~\unit{\micro\watt}$. The former requirement is satisfied using a reference cavity and a Pound-Drever-Hall (PDH) locking scheme. The latter requirement is satisfied through a closed feedback loop between a pick-off photodiode signal before the ion monitoring the optical power, and the drive RF amplitude of an upstream AOM.\\
...
372-nm: iBEAM-SMART-375-S.



    PDH locking is used to ensure that laser frequencies are listed above. The laser frequencies and lock status are continuously monitored with a wavemeter.\\
    Frequency and amplitude control for all lasers is achieved via acousto-optic modulators (AOMs) which are driven by \emph{Artiq} integrated DDS chips, \emph{Urukuls}. The AOMs are used in a double pass configuration to allow shifts of the laser frequency by $\pm \sim 100$~MHz, with no significant angular shifts or loss of fibre coupling. 
