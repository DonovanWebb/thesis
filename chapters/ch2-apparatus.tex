...
\subsection{Frequency Stabilisation}
    ...
    \begin{figure}
    \begin{center}
    \noindent\includegraphics[width=0.7\linewidth]{figures/pdf_figure/ch2/pdh.pdf}
    \end{center}
    ...
    techniques to generate the error signal for the feedback loop. The PDH technique is well described in the literature~\cite{drever1983laser, oswald2022characterization, black2001introduction}, and so here we only give a brief overview of the technique to contextualise the required hardware introduced in the following sections.\\
...
% Beam paths? AOM DP and SP?which are frequency stabilised to a reference cavity\footnote{\emph{Stable Laser Systems} SLS-6010 4-Bore Cylindrical Cavity} via Pound-Drever-Hall (PDH) locking. 
    ...
    applying both offset and PDH sidebands for an offset PDH lock. The offset lock allows for stabilising the output laser at an arbitrary frequency, rather than matching one of the cavity mode frequencies. For 397-nm
    ...
    parabolic mirror outcoupler\footnote{\emph{Thorlabs} RC02APC-F01} inside the MuMetal box, and care was taken for
...
% Pre reqs:
    ...
    to an ulta high finesse cavity\footnote{\emph{Stable Laser Systems} P/N 6020-4} and applying
    a PDH lock (see above section). 
    %\mccorrect{XXX should quote finesse and FSR values here.} 
    ...
    the environment, it is made of an ultra low expansion material, and mechanically isolated from the environment by being mounted on a vibration isolation platform. The cavity is housed in a vacuum system to prevent refractive index fluctuations due to air, and to further reduce thermal coupling to the environment. 
