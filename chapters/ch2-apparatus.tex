...
\begin{figure}
  \begin{center}
   \noindent\includegraphics[width=\linewidth]{figures/pdf_figure/ch2/trap_package.pdf}
  \end{center}
  \caption{
    \textbf{a)} 
    \textbf{b)} 
...
\begin{figure}
  \begin{center}
   \noindent\includegraphics[width=\linewidth]{figures/pdf_figure/ch2/HNA_package.pdf}
  \end{center}
  \caption{
    }
  \label{fig:HNA}
...
\begin{figure}
  \begin{center}
   \noindent\includegraphics[width=0.9\linewidth]{figures/pdf_figure/ch2/readout.pdf}
  \end{center}
...
\end{figure}
    ...
    distinguishability of ions in the imaged plane. Figure~\ref{fig:readout} shows the optical path schematic of the imaging system.
    We use a 
    high quantum-efficiency ($\simeq 70\%$ at $397~\unit{\nano\meter}$) 
    CMOS camera\footnote{\emph{Hamamatsu Orca-Fusion
    BT} C15440-20UP} for spatially selective qubit readout on a multi-ion chain. A typical signal of which can be seen in the upper right inset for an eight ion chain. We also divert $\simeq 10\%$ of the fluorescent light to a Photon multiplier tube (PMT)\footnote{\emph{Hamamatsu} Photon counting head H10682-210}, for experiments requiring time resolved photon counting (e.g. micromotion compensation, section~\ref{sec:Micromotion}), a typical signal of which can be seen in the lower right inset.
    The rest of the optical path consists of a high numerical aperture objective\footnote{\emph{Photon Gear, inc.} Custom 18WD Atom Imager} (effective focal length, $f=33~\unit{\milli\meter}$), $\mathrm{NA}=0.6$, a dichroic mirror\footnote{\emph{Thorlabs} TODO} (transmission: $397~\unit{\nano\meter}$; reflection: $729~\unit{\nano\meter}$), a relay lens\footnote{\emph{Thorlabs} Achromat Doublet, ACT508-200-A-ML} ($f=200~\unit{\milli\meter}$), a $90:10$ plate beamsplitter\footnote{\emph{Thorlabs} TODO}, and finallly the camera and PMT. \\
    The objective lens is custom designed to compensate for aberrations due to the recessed vacuum viewport, however, due to the large NA, accurate alignment of all degrees of freedom of the objective is critical. As such, the NA is attached to a 5-axis alignment stage\footnote{\emph{Newport ULTRAlign} M-562-XYZ, M-36, SM-13}, which can be seen in figure~\ref{fig:solidworks}. The z-axis focus is controlled by a piezo micrometer\footnote{\emph{Newport CONEX-TRA12CC}} for fine adjustment of the ion focus with the MuMetal box closed (requires $<\unit{\micro\meter}$ precision).\\
    Given the chosen lens focal lengths, we expect a magnification of $\simeq 6$ from the ion plane to the imaging plane. With an expected ion separation of $5~\unit{\micro\meter}$, and the camera pixel pitch of $6.5~\unit{\micro\meter}$, we expect around 4.5 pixels between ions, which, if the system is well aligned, is more than sufficient for low cross-talk spatial readout of qubits.
...
% Pre reqs:
    ...
    The qubit is defined by the quadrupole transition at 729-nm. Access to other excited levels, outside of our defined two level system, are crucial for ion trap quantum logic to enable qubit readout via state selective fluorescence (397-nm, and 866-nm transitions), and state preparation via optical pumping (397-nm, 866-nm, and 854-nm transitions). Access to two additional transitions in neutral calcium for isotope selective ionisation, 422-nm and 373-nm, are also required~\cite{lucas2004isotope-selective}.\\
    All transitions, except for 729-nm, are driven by diode lasers, described in the following section. Subsequently, the 729-nm source is discussed. Finally, the global and single addressed beam arrangements for the 729-nm system are detailed at the end of this section.\\
    % Add small section on PDH lock set up in general, and remove from subsequent sections.
    
\begin{figure}
  \begin{center}
   \noindent\includegraphics[width=\linewidth]{figures/pdf_figure/ch2/diode_paths.pdf}
  \end{center}
  \caption{
...
% Beam paths? AOM DP and SP?which are frequency stabilised to a reference cavity\footnote{\emph{Stable Laser Systems} SLS-6010 4-Bore Cylindrical Cavity} via Pound-Drever-Hall (PDH) locking. 
    ...
    dipole transitions, and power stable to $<1~\unit{\micro\watt}$. The former
    requirement is satisfied using a reference cavity and a Pound-Drever-Hall
    (PDH) locking scheme. The latter requirement is satisfied through a closed
    feedback loop between a pick-off photodiode signal before the ion monitoring
    the optical power, and the drive RF amplitude of an upstream AOM.\\
    ...
    applying both offset and PDH sidebands for an offset PDH lock. For 397-nm
    and 423-nm, resonant free space EOMs\footnote{\emph{Qubig} PM7-VIS-25, TODO}
    are used, for 854-nm and 866-nm broadband fibre EOMs\footnote{TODO} are used
    for convenience. The path on the reference cavity board
    (fig~\ref{fig:diode_paths} (c)) consists of a four-bore reference
    cavity\footnote{\emph{Stable Laser Systems} ...} under vacuum and
    ...
    reflected cavity light to a fast photodiode\footnote{\emph{Thorlabs} ...}.
    ...
    asymmetric PDH trace. Gains for PID controllers of both the diode current
    and voltage are set to stabilise the optical frequency.\\
    The light from
    the ``laser'' laboratory is carried to the ``trap" laboratory using 16~m
    polarisation maintaining fibres\footnote{\emph{Schafter-Kirchoff} ...}. In
    ...
    (fig~\ref{fig:diode_paths} (d) and (e)). The power stabilisation feedback
    loop consists of a signal proportional to the optical power measured by a
    low-noise photodiode\footnote{\emph{QLNPD} system designed by David
    Nadlinger, using \emph{Hamamatsu} ...} (optical pick-off in the MuMetal
    enclosure fig~\ref{fig:diode_paths} (h)), being sampled using an ADC
    (\emph{Sinara Sampler}), and feeding back to the output amplitude of the DDS
    RF source (\emph{Sinara Urukul}) that drives the double pass
    AOM\footnote{This feedback system, known as \emph{SUServo}, was written by
    Peter Drmota.}.\\
    As space inside the MuMetal enclosure is constrained, it is advantageous the
    ...
    takes the light directly into the MuMetal enclosure. The PI light uses a
    parabolic mirror outcoupler inside the MuMetal box, and care was taken for
    the beam propagation to be perpendicular to both the lens and vacuum
    viewport window as it was found that the difference in refractive index of
    the optical materials at these two wavelengths was significant enough to
    cause the two beams to be separated by more than one waist at the ion plane
    after initial rough alignment. The 854-nm and 866-nm light is combined after
    their respective double and single pass AOMs, and coupled into a 3~m PM\footnote{\emph{SK} ...}
    fibre carrying the light into the enclosure.\\
    Light from both the PI and 397/854/866-nm in-box path is directed onto the
    ions via the in-vacuum prisms, discussed in section~\ref{sec:Vacuum System}.
    The final mirrors prior to the vacuum system are
    motorised\footnote{\emph{Newport} ...}, so that beam alignment to the ions
...
% Pre reqs:
    ...
    single mode polarization maintaining fibre\footnote{\emph{SK} ...}.  The fibre is
    ...
    stabilisation by a fibre-noise-cancellation (FNC) technique~\cite{ma1994delivering}, is used\footnote{FNC control consists of \emph{Sinara Stabilizer} and \emph{Pounder}
    mezzanine board with PID software 
    developed by Ayush Agrawal.}. A comparison of spin coherence times  
...
% Pre reqs:
    ...
    report~\cite{oevergaard2025transfer}, availible upon request. \\
    %
    Multiple methods for single ion addressing have been developed in the ion
    trap community, including waveguide arrays~\cite{sotirova2024high-fidelity,
    integrated}, micro-mirrors~\cite{}, spatial-light-modulators~\cite{}, and
    acousto-optic deflectors~\cite{}. Each method has its own advantages and
...
4:1 aspect ratio, with major axis along the ion chain direction. The resulting
major axis has a beam waist radius of $52(3)~\unit{\micro\meter}$\footnote{The
...
% Main points:
    ...
    in the resistive tube oven described in section~\ref{sec:Vacuum System}. The
    thermocouple, tag welded onto the tube oven, provides a temperature reading
    for current feedback to reach a target temperature of $\simeq
    200^\circ\mathrm{C}$. This requires $\simeq 2~\unit{A}$
    across the $0.5~\unit{\ohm}$ load of the oven tube. The tube oven has a
    pinhole that is aligned to the ion flux aperture, shown in
    figure~\ref{fig:atom_flux}, and the atomic shield pinhole shown in
    figure~\ref{fig:trap}. These apertures were included in the design to
    improve the lifetime of the trap by ensuring neutral Calcium metal cannot
    build up on the trap surface and cause shorts between RF, ground, and DC
    ...
    too large. It was \emph{lucky} as the planned trapping region of dc10 became
    non-viable after electrode Fr dc11 developed a continuity fault (which we
    believe is in-vacuum). As such, ions are loaded at dc5.\\
    The neutral atom flux is ionised using a two-step photoionisation
    scheme~\cite{}. The first stage involves a frequency stabilised 423-nm beam
    that couples the 4S->XX transition in neutral calcium. The isotope shift is
    XX, which is much larger than the transitions natural linewidth and the
    lasers linewidth, as such the desired isotope can be selectively loaded. We
    use natural abundance Calcium, with typical isotope proportions of
    $XX\%:\mathrm{^{40}Ca},~XX\%:\mathrm{^{42}Ca},~XX\%:\mathrm{^{43}Ca}$. In
    this work we use exclusively \ca, and so set the 423-nm to excite neutral
    $\mathrm{^{40}Ca}$ only\footnote{We practically see no loading of other
    ...
    $\mathrm{CaH}^+$).}. The second ionisation stage couples XX->continuum using
    a 373-nm UV laser source (although the exact wavelength does not matter as
    long as <xxx-nm).\\
    The 373-nm laser can cause charge build-up on the trap electrodes, and so we
    use a physical shutter to block the beam when not actively loading ions.
