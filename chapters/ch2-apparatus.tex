...
% Figures
    ...
    noise.  
    The circuit diagram of the constructed resonator~\footnote{Credit to Emily Hirsch for construction of the recent resonator.} is shown in figure~\ref{fig:res_H}. The inductor consists of XX(ToDo gauge and oxygen?) gauge Copper was made to have, $L\sim 0.8~\unit{\micro\henry}$, with toroidal iron powder core\footnote{Micrometals T80-6}, and C0G (low tempco) rated ceramic capacitors, $C=5.4~\unit{\pico\farad}$. The vacuum can feedthrough and trap were measured to present an $\sim 18~\unit{\pico\farad}$ load. The trap itself presents a $C_T=4~\unit{\pico\farad}$~\cite{wilpers2013compact}, the cable between the resonator and the vacuum feedthrough contributes $\sim 4~\unit{\pico\farad}$, and the remaining presented capacitance is expected to originate from the feedthrough and in-vacuum RF line.
    The resonator had a measured  
    quality factor of $Q=66.7(X)$(ToDo: get quality factor plot), and a central frequency of $36.5(X)~\unit{\mega\hertz}$.
    The effect of the resonator on the AM and FM noise can be calculated through the transfer function, $H(f)$, of the resonator circuit. 
    Capacitors ($C2$ and $C3$) are used to provide a pick-off (p.o.) voltage which can be monitored, and if required, used in a closed feedback loop. We however have not attempted this as of yet, and shall ignore the effect of the pick-off for this analysis. The transfer function of the simplified circuit is then given by,
    \begin{equation}
    ...
    find that for both AM$\rightarrow$AM and FM$\rightarrow$FM (where "$\rightarrow$" represents the transform from input noise type to output noise type) the input PSD are
