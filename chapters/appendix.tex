...
\label{app:magnus}
    ...
    dependent exponent, which is expanded in the Magnus construction to,
    \begin{equation}
    ...
    Given the ``two-SDF'' Hamiltonian in equation~\ref{eqn:two-sdf}, reproduced here,
    \begin{align}
    \label{eqn:two-sdf}
    ...
    \end{align}
    we may
    evaluate $A(t)$ to a chosen Magnus order. We shall consider the first two terms explicitly here, with similar arguments possible for higher order. \\
    We can immediately make two simplifying assumptions: first concerns the fast
    oscillating $A_1$ term,
    \begin{equation}
        A_1 = ...
    \end{equation}
    This looks like separate spin dependent phase-space
    loops originating from the off-resonant SDFs. As these loops close with
    periodicity $\tau_\alpha = 2\pi/\Delta_\alpha$ and $\tau_\beta = 2\pi/\Delta_\beta$, we may select a gate duration $T$ such
    that $T=p\tau_\alpha=q\tau_\beta$, to enforce $A_1(T)=0$. \\
    Second, we may further expand $A_2$ to reveal 4 components,
    \begin{equation}
    A_2 = \int_{0}^{t}\int_{0}^{t_1} (S_\alpha + S_\beta + X_\mathrm{pair} + X_\mathrm{exch}),
    \end{equation}
    where $S$ ``self-terms'', and $X$ ``cross-terms''. The self-terms, with the form
    \begin{equation}
        S_i = ...,
    \end{equation}
    ...
    These remaining cross-terms contain the 2nd order non-linear bosonic interactions we are interested in emulating.
    To recover the general family of non-linear bosonic interactions given in table~\ref{tab:gateset},
    motional phases are set to $\phi_\alpha=0$, $\phi_\beta=\phi$, and spin-conditioning is assumed to be the anti-commuting pair: $\sigma_\alpha=\sigma_x$,
    $\sigma_\beta = \sigma_y$. We set the detunings of the two SDF to be $\Delta_\beta = m\Delta_\alpha$, where $m$ is
    some integer. 
    Expanding the commutator of the cross terms we find,
    \begin{align}
        X_\mathrm{pair} &= ,\\
        X_\mathrm{exch} &= ,\\
    \end{align}
    where...
    We see now the freedom to ``tune'' the desired interaction, if we set $m=+1$, we resonanly drive..., alternatively if we set $m=-1$, we recover...
    We can convert from the generator form, $A(T)$, to familiar effective 
    time-independent Hamiltonian, $H_\mathrm{eff}$ for a given time, $T$, by $\hat H_\mathrm{eff} = \frac{i\hbar}{T}A(T)$.\\
    For $m=+1$, we find that the effective Hamiltonian is,
    \begin{equation}
        H_\mathrm{eff, exch} = ...
    \end{equation}
    which for a single mode, $a^\dagger = b^\dagger$, is the parity operation, and for two modes, is the beam splitter.\\
    For $m=-1$, we find,
    \begin{equation}
        H_\mathrm{eff, pair} = ...
    \end{equation}
    which for a single mode, $a^\dagger = b^\dagger$, is the squeezing operation, and for two modes, is the two-mode squeezing.\\
    Similar arguments hold for $m=2$, where now the $A_2$ cross terms appear off-resonant by..., and the on resonant cross terms in $A_3$ coherently sum over time yielding effective 3rd order bosonic operators, such as the tri-squeezing interaction.\\
    Higher order error term scaling...