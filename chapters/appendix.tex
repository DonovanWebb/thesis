...
\section{Detuning errors}
To exemplify the method for calculating the effect of an error channel on a single qubit gate, we consider the simple case of a coherent detuning error. We define the Fidelity of a quantum channel $\mathcal{E}$ with respect to a unitary $U$ as
\begin{equation}
    \label{eqn:fid}
...
\begin{equation}
    ...
    F(V,U) = \frac{1}{d(d+1)} \left( |\text{Tr}(W)|^2 + d \right),
\end{equation}
 where $V$ is the error unitary, $W=U^\dagger V$ and $d$ is the dimension of the Hilbert space. \\
...
Considering the explicit case of a $R_x(\pi/2)$ gate with a coherent
detuning error, we can write the error unitary as 
\begin{equation}
V = \text{exp}\left(-i\frac{\pi}{4}\left(\hat\sigma_x + \frac{\delta}{\Omega}\hat\sigma_z\right)\right),
\end{equation}
where $\delta$ is the detuning from resonance, and $\Omega$ is the Rabi frequency. The ideal unitary is given by $U = \text{exp}(-i\frac{\pi}{4}\hat\sigma_x)$, yielding trace of the product $W = U^\dagger V$ as
\begin{equation}
    \text{Tr}(W) = 2 - \frac{1}{2}\left(\frac{\delta}{\Omega}\right)^2 + \mathcal{O}\left(\frac{\delta}{\Omega}\right)^4,
\end{equation}
where we have assumed the detuning is small compared to the Rabi frequency. Inserting this into equation~\ref{eqn:fid_unitary}, we find the fidelity of the detuned gate is given by
\begin{equation}
    F(V,U) = 1 - \frac{1}{3}\left(\frac{\delta}{\Omega}\right)^2 + \mathcal{O}\left(\frac{\delta}{\Omega}\right)^4.
\end{equation}
To convert this to an error contribution to a single qubit Clifford gate, we introduce the scalar $\kappa$, which depends on the specific decomposition of the Clifford gate set into the native gate set of the system. The error contribution is then given by
\begin{equation}
    \epsilon = \kappa(1-F(V,U)) = \frac{\kappa}{3}\left(\frac{\delta}{\Omega}\right)^2 + \mathcal{O}\left(\frac{\delta}{\Omega}\right)^4.
\end{equation}
