...
\subsection{``Two-SDF'' dynamics}
    ...
    \begin{align}
        X_\mathrm{pair} &= \frac{i\epsilon_{\alpha\beta\gamma} \Omega_\alpha\Omega_\beta  }{4\Delta }\hat\sigma_\gamma \left(\hat a\hat b e^{i\phi_p}J(m,t)+\hat a^{\dagger}b^\dagger e^{-i\phi_p}J(m,t)^\dagger\right),\\
        X_\mathrm{exch} &= \frac{i\epsilon_{\alpha\beta\gamma}\Omega_\alpha\Omega_\beta }{8\Delta}\left(\{\hat a, \hat b^\dagger\}e^{i\phi_m}F(m,t) + \{\hat a^\dagger, \hat b \} e^{-i\phi_m}F(m,t)^\dagger\right),
    \end{align}
    ...
    \begin{equation}
        \hat H_\mathrm{eff, pair} = \frac{\varepsilon_{\alpha\beta\gamma}\hbar \Omega_2}{2}\hat\sigma_\gamma\left(\hat a \hat b e^{i\phi_p'}+\hat a^\dagger \hat b^\dagger e^{-i\phi_p'}\right),
    \end{equation}
    where $\Omega_2 = \frac{\Omega_\alpha\Omega_\beta}{\Delta}$, and $\phi_p' = \phi_\alpha +\phi_\beta + \frac{\pi}{2}$. For $a^\dagger = b^\dagger$ this is the squeezing interaction, whereas for $a^\dagger \neq b^\dagger$ this is two-mode squeezing.\\
    Taking the limit, $m=+1$, we find $J(1,t)=0$ and $F(1,t) = -2it\left(1-\mathrm{sinc}(\Delta t)\right)$. If we consider the secular term only, we find the time-linear term $X_\mathrm{exch}$. Now considering the anticommutation relationships between ladder operators we find for $a^\dagger = b^\dagger$, an effective Hamiltonian,
    \begin{equation}
        \hat H_\mathrm{eff, parity} = \frac{\varepsilon_{\alpha\beta\gamma}\hbar \Omega_2}{2}\hat\sigma_\gamma\sin(\phi_m) (2\hat a^\dagger \hat a +1),
    \end{equation}
    which is the parity, or in this case ``dispersive'' Hamiltonian due to the spin-conditioning.\\
    Finally, considering $a^\dagger \neq b^\dagger$, we recover the beamsplitter interaction, with effective Hamiltonian,
    \begin{equation}
        \hat H_\mathrm{eff, bs} = \frac{\varepsilon_{\alpha\beta\gamma}\hbar \Omega_2}{2}\hat\sigma_\gamma \left(\hat a \hat b^\dagger e^{i\phi_m'}+\hat a^\dagger \hat b e^{-i\phi_m'}\right),
    \end{equation}
    ...
    \begin{equation}
        \hat H_\mathrm{eff, tri} = \frac{\hbar \Omega_3}{2}\hat\sigma_\beta \left(\hat a^3 e^{i\phi}+\hat a^{\dagger 3}  e^{-i\phi}\right),
    \end{equation}
