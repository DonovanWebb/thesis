...
% Pre reqs:
    ...
    The \ca~ion has a rich level structure, shown in figure~\ref{fig:ion}, with
...
\todo{Populate this section with outline of calibrations?}
\subsubsection{Micromotion Compensation}
\label{sec:Micromotion}
%TODO

\subsubsection{Magnetic Field and Laser Offset}
\label{sec:Magnetic Field and Laser Offset}
%TODO 

\subsubsection{Beam Pointing Calibration}
\label{sec:Beam Pointing Calibration}
%TODO General stark shift experiments?
...
\todo{Extend section, move theory section to ch2, and conform with k-copy style\\}
    ...
    (RB)~\cite{knill2008randomized} is employed.  RB consists of
    ...
    conjugation. The RB protocol used here is described in~\cite{hughes2021benchmarking}.
...
\subsection{Robust Phase Estimation}
\label{sec:RPE}
\todo{Use this method a lot, would be good to describe?\\}

\subsection{sec:Single Addressing char}
\label{sec:Single Addressing char}
\todo{Short section presenting results --- maybe move to apparatus?\\}
...
% Pre reqs:
    ...
    % Available transitions
    % 729 laser system
    \todo{Add section of spin-lock scans of laser.}
    \begin{figure}
        \begin{center}
        \noindent\includegraphics[width=\linewidth]{
...
% Pre reqs:
    ...
    ion, the radial mode frequencies are $\omega_{\rm upper}/2\pi =
...
\todo{Currently this contains the tickle protocol, which is also used for general calibrations. Ref in ch6}
\todo{Requires plot and main text numbers update, but we dont yet have data...\\}
% Main points:
